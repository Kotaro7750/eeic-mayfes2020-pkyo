...
function initGlobals(){
    ...
    commandGenerator = (function* (){return;})();
}
...
//メインループ
//一定時間ごとに再実行される
let waitForNextCommand=0;
function mainLoop(){
    //マウス及びキーボード入力をmainLoop関数と同期させる
    keyboardManager.propagate();
    mouseManager.propagate();

    //キー入力に呼応してアイテムを動かす(blocklyで動かすので不要)
    //moveItems();


    let gen=commandGenerator.next();
    if (!gen.done){
        if (waitForNextCommand<=0){
            while (!gen.done){
                let command = gen.value.split(' ');
                let immediate=false;
                console.log(command);
                switch (command[0]){
                    case "MOVE":
                        mainField.moveItem(0, command[1]);
                        break;
                }
                if (!immediate){
                    waitForNextCommand=10;
                    break;
                }
            }
        }else{
...
Blockly.JavaScript['move'] = function(block) {
    ...
    return 'yield "MOVE '+dropdown_direction+'";\n';
};
...
function runCode() {
    ...
    try {
        //eval('(async () => {' + code + '})();');
        commandGenerator = eval('(function* (){\n'+code+';\nreturn;})')();
    } catch (err) {
        alert(err);
    }
}
