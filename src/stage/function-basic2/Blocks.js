...
export default {
  ...
  block_function: (block) => {
    ...
    const code1 = Blockly.JavaScript.statementToCode(block, 'CODE1');
    const code2 = Blockly.JavaScript.statementToCode(block, 'CODE2');
    return `
    funcs['${group}'] = function*(interlude) {
      this.cmdDelta = 35;
      yield "${block.id.replace(/"/, '\\"')}";
      ${code1}
      {
        let tmp_gen = interlude();
        while(true) {
  ...
  block_functioncall: (block) => {
    ...
    return `{
      let tmp_gen = funcs['${group}'](function*() {
        ${code}
      }.bind(this));
      this.cmdDelta = 35;
      yield "${block.id.replace(/"/, '\\"')}";
